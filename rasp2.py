...
class EggDetector:
    ...
    def connect_camera(self):
        try:
            ...
            if self.camera is not None and isinstance(self.camera, cv2.VideoCapture):
                self.camera.release()

            if self.use_picamera:
                # Skip PiCamera code on Windows - only relevant for Raspberry Pi
                if os.name != 'nt':  # Check if not on Windows
                    ...
                    # On Windows, don't try to use picamera
                    self.use_picamera = False
                    print("Running on Windows, PiCamera not supported")
            ...
            if os.name == 'nt':  # Check if on Windows
                ...
                print(f"Connecting to camera {self.camera_index} using DirectShow backend")
            else:
                self.camera = cv2.VideoCapture(self.camera_index)
                print(f"Connecting to camera {self.camera_index} using default backend")

            if not self.camera.isOpened():
                # Try a few more indices on Windows before giving up
                if os.name == 'nt':
                    for i in range(10):
                ...
                if not self.camera.isOpened():
                    logging.error(f"Could not open any camera")
                    return False

            # Set camera properties for better performance
            self.camera.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
            self.camera.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            # Set a lower FPS for better stability
            self.camera.set(cv2.CAP_PROP_FPS, 15)

            # Read a test frame to make sure it works
            ret, test_frame = self.camera.read()
            if not ret:
                logging.error("Camera opened but could not read frame")
                return False

            print(f"Successfully connected to camera at index {self.camera_index}")
            return True
